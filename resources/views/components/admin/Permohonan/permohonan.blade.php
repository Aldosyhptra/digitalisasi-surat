...
<div class="bg-white p-6 w-full max-w-7xl mx-auto mb-6">
    <div class="flex flex-wrap gap-6 items-start">
        ...
        @php
            $filters = [
                [
                    'id' => 'filter-status',
                    'label' => 'Filter Status',
                    'options' => ['' => 'Semua Status', 'Baru' => 'Baru', 'Proses' => 'Proses', 'Selesai' => 'Selesai'],
                ],
                [
        ...
        @foreach ($filters as $filter)
            <div>
                <label for="{{ $filter['id'] }}"
                    class="text-gray-700 text-sm font-medium mb-1 block">{{ $filter['label'] }}</label>
                <div class="relative w-[223px]">
                    <select id="{{ $filter['id'] }}"
                        class="w-full h-10 px-3 border border-gray-300 bg-white text-gray-600 text-sm rounded-lg">
...
{{-- Card Tabel --}}
<div
    class="bg-white shadow-lg rounded-2xl p-4 w-full max-w-7xl mx-auto overflow-x-auto font-inter text-[14px] text-[#111827]">
    ...
    <div class="min-w-[900px]" id="table-body">
        ...
        @foreach ($rows as $item)
            <div class="row-item grid grid-cols-5 gap-6 items-center bg-gray-50 p-3 rounded-xl">
                ...
                <div
                    class="status rounded-lg px-3 py-1 font-semibold text-center w-fit text-[12px]
            {{ $statusStyles[$item->status] ?? 'bg-gray-100 text-gray-800' }}">
                ...
                <div class="flex gap-3 actions">
                    <a href="{{ route('admin.riwayat-pengajuan.detail', $item->id) }}"
                        class="text-blue-600 font-semibold hover:underline text-center">
                        Lihat & Proses
                    </a>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderTable() {
            ...
            if (totalPages > 1) {
                ...
                prevBtn.className =
                    'flex items-center justify-center w-8 h-8 border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-40';
                prevBtn.innerHTML = `
