    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <div class="relative">
                ...
                <select disabled
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 bg-gray-50 appearance-none capitalize">
                    <option>{{ $user->jenis_kelamin ?? 'Belum Diisi' }}</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Tanggal Lahir</label>
                <input disabled value="{{ $user->tanggal_lahir ?? '-' }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 bg-gray-50">
            </div>
